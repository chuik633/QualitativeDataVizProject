...
function tooltip_help_me_code(x,y, text){
    d3.select("#tooltip").style('left', x+"px").style('top', y+"px").text(text)
}
function tooltip2_help_me_code(x,y, text){
    d3.select("#tooltip2").style('left', x+"px").style('top', y+"px").text(text)
}
function modify_nearby_blocks(svg, mouseX, mouseY, display_mode){
    ...
    svg.selectAll('.svg-image').each((img)=>{
        ...
        update_color_d(svg.select(`#color-${img.entry.id}`), distance, display_mode)
        
    })
}

// function update_image_d(img, distance){
//     //update the image size
//     if(distance < detection_threshold){
//         //compute the new size based on the distance
//         const curr_size = img.img_width
//         const scaling_facor = (detection_threshold - distance)/detection_threshold;
//         const new_size = Math.max(min_size,Math.min(max_size, curr_size+ scaling_facor*(max_size -curr_size)));
    
//         //updating the actual data
//         img.img_width = new_size;
//         img.img_height = new_size;


//         //updating the image
//         d3.select("#"+img.id).transition()
//             .duration(200)
//             .attr('width', new_size)
//             .attr('height', new_size)

//     }else{
//         img.img_width = min_size;
//         img.img_height = min_size;

//         d3.select("#"+img.id).transition().duration(200).attr('width', min_size).attr('height', min_size)
...
function update_size_d(block, id, distance){
    if(distance < detection_threshold){
        ...
        d3.select("#"+id).transition().duration(200).attr('width', min_size).attr('height', min_size)
...
function update_color_d(block, distance, display_mode){
    ...
    if(distance < detection_threshold){
        if(display_mode == "images" || display_mode == 'reveal'){
            const opacity_delta = (1 - (distance/width))*.08
